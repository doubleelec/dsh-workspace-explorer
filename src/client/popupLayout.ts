 ...
 */
export function autoPopupHeight(top: number, bottomLimit: number, vh: number, minH = POPUP_MIN_H): number {
 ...
 */
export function clampPopupHeight(h: number, top: number, vh: number, minH = POPUP_MIN_H): number {
  const maxH = Math.max(minH, vh - top - 16)
 ...
 */
export function loadManualHeight(key = POPUP_MANUAL_KEY): number | null {
  try {
 ...
 */
export function saveManualHeight(h: number | null, key = POPUP_MANUAL_KEY): void {
  try {
